...
library Manager {
    ...
    function newLibrary(address _library, bytes4[] calldata _functions) external {
        ...
        for (uint256 i = 0; i < _functions.length; i++) {
            if (DS.libraries[_functions[i]] != address(0)) {
                revert DuplicateFunction(_functions[i], DS.libraries[_functions[i]]);
            }
    ...
    function addFunctions(address _library, bytes4[] calldata _functions) external {
        ...
        for (uint256 i = 0; i < _functions.length; i++) {
            _f = _functions[i];
            if (DS.libraries[_f] != address(0)) revert DuplicateFunction(_f, DS.libraries[_f]);
    ...
    function removeFunctions(address _library, bytes4[] calldata _functions) external {
        ...
        while (len > 0) {
            _f = _functions[--len];
            if (DS.libraries[_f] != _library) revert DuplicateFunction(_f, DS.libraries[_f]);
    ...
    function replaceFunctions(address _src, address _dst, bytes4[] calldata _functions) external {
        ...
        //}
        for (uint256 i = 0; i < _functions.length; i++) {
            if (DS.libraries[_functions[i]] != _src) revert InvalidFunction(_functions[i]);
